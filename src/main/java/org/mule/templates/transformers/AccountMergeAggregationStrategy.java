...
public class AccountMergeAggregationStrategy implements AggregationStrategy {
	...
	public MuleEvent aggregate(AggregationContext context) throws MuleException {
		...
		List<Map<String, String>> mergedAccountList = accountMerge.mergeList(listA, listB);
	...
	private List<Map<String, String>> getAccountsList(List<MuleEvent> events, int index) {
		...
		if (events.get(index).getMessage().getPayload() instanceof List)			
			iterator = ((List<Map<String, String>>) events.get(index).getMessage().getPayload()).iterator();
		else
